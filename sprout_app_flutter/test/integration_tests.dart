...
import 'package:sprout_app/main.dart' as app;
import 'package:sprout_app/services/project_service.dart';
import 'package:sprout_app/services/debugger.dart';
import 'package:sprout_app/services/reactive_runtime.dart';
...
void main() {
  ...
  group('Sprout App Integration Tests', () {
    ...
    testWidgets('Reactive runtime operations', (WidgetTester tester) async {
      ...
      runtime.computed('sum', () {
        ...
        final x = runtime.getValue('x', 0);
        final y = runtime.getValue('y', 0);
        return x + y;
      }, ['x', 'y']);
  ...
  group('Error Handling and Recovery', () {
    testWidgets('Handle invalid project names', (WidgetTester tester) async {
      final projectService = ProjectService();
      expect(() async => await projectService.createProject(''),
          throwsA(isA<ProjectException>()));
    });

    testWidgets('Compiler error handling', (WidgetTester tester) async {
      final projectService = ProjectService();
      expect(() async => await projectService.compileCode('bad { syntax'),
